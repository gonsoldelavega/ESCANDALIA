...
select
  ...
  d.pvp,
  d.published,
  coalesce(sum(di.quantity * i.current_cost), 0)::numeric(12,4) as cost_total,
  case when d.pvp > 0 then ((d.pvp - coalesce(sum(di.quantity * i.current_cost), 0)) / d.pvp)::numeric(8,4) else 0 end as gross_margin,
  case when d.pvp > 0 then ceil((coalesce(sum(di.quantity * i.current_cost), 0) / (1 - b.target_margin)) * 20) / 20 else 0 end::numeric(10,2) as recommended_pvp
...
group by d.id, d.business_id, d.name, d.category, d.pvp, d.published, b.target_margin;
